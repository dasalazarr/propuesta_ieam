...
// Simple class merger if @/lib/utils doesn't exist or we want to be self-contained for now
...
export interface CardProps {
  variant?: CardVariant;
  image?: string;
  badge?: string; // e.g., "Informe", "Evento"
  badgeColor?: string; // Optional override
  title: string;
  summary?: string;
  metadata?: {
    date?: string;
    ...
    readTime?: string;
    location?: string;
    format?: string; // e.g., "Presencial", "Online"
  };
  ctaText?: string;
  ctaLink?: string;
  className?: string;
...
export const Card: React.FC<CardProps> = ({
  ...
  image,
  badge,
  badgeColor = 'bg-[#0A2540]', // Default Navy
  title,
  summary,
  metadata,
  ctaText,
  ...
  // Icon helper based on badge text (optional enhancement)
  const getIcon = () => {
  ...
  const containerProps = ctaLink ? { href: ctaLink } : { onClick };
    ...
    return (
      <Container
        ...
        className={classNames(
          "group flex gap-4 p-4 border-b hairline hover:bg-slate-50 transition-colors cursor-pointer",
          className
        )}
      >
        {metadata?.date && (
  ...
  return (
    <Container
      {...containerProps}
      className={classNames(
        "group block bg-white overflow-hidden border hairline hover:-translate-y-1 transition-all duration-300",
        isFeatured ? "grid md:grid-cols-12 gap-0" : "flex flex-col h-full",
        className
      )}
    >
      {/* Image Section */}
          ...
          {badge && !isFeatured && (
            <div className="absolute top-4 left-4">
              <span className={classNames(
                "inline-flex items-center px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider shadow-sm border rounded-full",
                badgeTheme.bg,
                badgeTheme.text,
                badgeTheme.border
              )}>
      ...
      <div className={classNames(
        ...
        isFeatured ? "md:col-span-5 bg-gradient-to-br from-white to-[#F2F6FB]" : "flex-grow"
      )}>
        <div>
          {!isFeatured && (
            <div className="h-1 w-12 bg-[#006994] mb-4 rounded-full" />
          )}
          {/* Meta Header */}
          ...
          <h3 className={classNames(
            "font-serif font-bold text-slate-900 group-hover:text-[#006994] transition-colors mb-3",
